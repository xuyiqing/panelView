...
library(dplyr)
capacity %>% panelview(Capacity ~ 1, index = c("ccode","year"), axis.lab="off", type = "miss")

capacity %>%  panelview(Capacity ~ demo, ignore.treat = TRUE, index = c("ccode","year"), axis.lab= "off")


## plot with outcome variable
capacity %>% panelview(Y = "Capacity", index = c("ccode", "year"), axis.lab.gap = c(2,10), type = "miss")
...
panelview(turnout ~ policy_edr + policy_mail_in + policy_motor, data = turnout, index = c("abb","year"), type = "outcome", main = "EDR Reform and Turnout", theme.bw = FALSE)

## change colors
panelview(turnout ~ policy_edr + policy_mail_in + policy_motor, data = turnout, index = c("abb","year"), type = "outcome", main = "EDR Reform and Turnout", color = c("#FC8D6280", "red", "#99999950"), legend.labs = c("Control States","Treated States (before EDR)", "Treated States (after EDR)"), theme.bw = FALSE)
...
panelview(lnpop ~ polity2, data = capacity, index = c("country", "year"),
                    ylim = list(c(8.75,9.4), c(-0.2, 0.35)),
          type = "bivar")
